...
def saturation_curve(results_preds, results_stats, concept):
    ...
    lits_pats = results_stats.sort_values('Rc',ascending=False).index.tolist()
    cumm_pats = []
...
def prep_template(concept, db, json=True):
    if json:
        print(
        '{\n'
        '  "target_rules": [\n'
    ...
    for l in db.loc[db['label']==concept]['literal'].drop_duplicates().sort_values():
        print(l)
